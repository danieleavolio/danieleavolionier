-- Fresh Supabase schema for the Daniele Avolio content CMS.
...
create trigger now_items_set_updated_at before update on public.now_items for each row execute function public.set_updated_at();
...
create policy "Public published posts are readable" on public.posts for select to anon, authenticated using (published = true);
drop policy if exists "Admins read posts" on public.posts;
create policy "Admins read posts" on public.posts for select to authenticated using ((select private.is_admin()));
...
create policy "Public published projects are readable" on public.projects for select to anon, authenticated using (published = true);
drop policy if exists "Admins read projects" on public.projects;
create policy "Admins read projects" on public.projects for select to authenticated using ((select private.is_admin()));
drop policy if exists "Admins manage projects" on public.projects;
create policy "Admins manage projects" on public.projects for all to authenticated using ((select private.is_admin())) with check ((select private.is_admin()));

drop policy if exists "Public published Now items are readable" on public.now_items;
create policy "Public published Now items are readable" on public.now_items for select to anon, authenticated using (published = true);
drop policy if exists "Admins read Now items" on public.now_items;
create policy "Admins read Now items" on public.now_items for select to authenticated using ((select private.is_admin()));
drop policy if exists "Admins manage Now items" on public.now_items;
create policy "Admins manage Now items" on public.now_items for all to authenticated using ((select private.is_admin())) with check ((select private.is_admin()));

drop policy if exists "Admins can read their admin record" on public.admin_users;
create policy "Admins can read their admin record" on public.admin_users for select to authenticated using (user_id = auth.uid());
