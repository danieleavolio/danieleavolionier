	...
	{:else}
		...
		<div class="editor-grid">
			<form method="POST" use:enhance>
				...
				<textarea id="description" name="description" bind:value="{selectedPost.description}" required></textarea>

				<label for="categories">Categorie (separate da virgola)</label>
				<input type="text" id="categories" name="categories" bind:value="{selectedPost.categories}" required />

				<label for="content">Contenuto (Markdown)</label>
				<textarea id="content" name="content" rows="20" bind:value="{selectedPost.content}" required></textarea>
